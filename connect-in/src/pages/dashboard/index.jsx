                                    ...
                                    return (
                                        <div key={post._id} className={styles.singleCard}>
                                            <div className={styles.singleCard_profileContainer}>
                                                ...
                                                }} src={`${BASE_URL}/${post.userId.profilePicture}`} className={styles.userProfile} alt="" />
                                                <div >
                                                    <div style={{ display: "flex", gap: "1.2rem", justifyContent: "space-between" }}>
                                                        <p  onClick={()=>{
                                                    router.push(`/view_profile/${post.userId.username}`)
                                                }}  style={{ fontWeight: "bold" , cursor:"pointer"}}>{post.userId.name}</p>
                                                        {post.userId._id === authState.user.userId._id && <svg onClick={async () => {
                                                            await dispatch(deletePost({
                                                                postId: post._id,
                                                                token: localStorage.getItem("token")
                                                            }))
                                                            await dispatch(getAllPosts());
                                                        }} style={{ height: "1.4em", color: "red", cursor: "pointer" }} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                                                    ...
                                                    <p  onClick={()=>{
                                                    router.push(`/view_profile/${post.userId.username}`)
                                                }} style={{ color: "grey" , cursor:"pointer"}}>@{post.userId.username}</p>
                                                    ...
                                                    <div className={styles.optionsContainer}>
                                                        ...
                                                        <div onClick={async () => {
                                                            ...
                                                            <svg style={{ cursor: "pointer" }} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                                                        ...
                                                        <div className={styles.singleOption_optionContainer}>
                                                            <svg onClick={() => {
                                                                ...
                                                                const url = encodeURIComponent("amazon.in");
                                                                const twitterURL = `http://twitter.com/intent/tweet?text=${text}&url=${url}`
                                                                window.open(twitterURL, "_blank")
                                                            }} style={{ cursor: "pointer" }} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
